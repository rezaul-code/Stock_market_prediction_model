...
class SmartSignalEngine:
    ...
    def _trend_signal(self, ema50: float, ema200: float, adx: float) -> str:
        "Detect market trend: Bull/Bear/Sideways."
    ...
    def _strict_signal_rules(self, pred_pct: float, rsi: float, macd_bull: bool, uptrend: bool) -> str:
        "Exact strict signal rules per task spec."
        if pred_pct > 2 and rsi < 70 and macd_bull and uptrend:
            return 'STRONG_BUY'
        elif pred_pct > 1 and rsi < 80 and macd_bull and uptrend:
            return 'BUY'
        elif pred_pct < -2 and rsi > 30 and not macd_bull and not uptrend:
            return 'STRONG_SELL'
        elif pred_pct < -1 and rsi > 20 and not macd_bull and not uptrend:
            return 'SELL'
        else:
            return 'HOLD'

    def generate_smart_signal(self, symbol: str = 'RELIANCE.NS') -> Dict:
        "Enhanced smart signal with strict rules + scoring fallback."
        ...
        # Strict rules first (task exact spec)
        macd_bullish = indicators['macd'] > indicators['macd_signal'] and indicators['macd_hist'] > 0
        uptrend = indicators['ema50'] > indicators['ema200']
        strict_signal = self._strict_signal_rules(pred_pct, indicators['rsi'], macd_bullish, uptrend)
        
        # Existing scoring as enhancement/fallback
        ...
        pred_signal_score = 'STRONG_BUY' if pred_pct > 2 else 'BUY' if pred_pct > 1 else \
                      'STRONG_SELL' if pred_pct < -2 else 'SELL' if pred_pct < -1 else 'HOLD'
        score = self.signal_strength[pred_signal_score] + \
                (2 if trend == 'BULLISH' else -2 if trend == 'BEARISH' else 0) + \
                (1 if momentum == 'BULLISH' else -1 if momentum == 'BEARISH' else 0)
        
        score_signal = 'STRONG_BUY' if score >= 6 else 'BUY' if score >= 3 else \
                      'STRONG_SELL' if score <= -6 else 'SELL' if score <= -3 else 'HOLD'
        
        # Use strict if strong match, else score
        final_signal = strict_signal if strict_signal in ['STRONG_BUY', 'STRONG_SELL'] else score_signal
        
        # Enhanced confidence: pred + vol + trend + model_agreement placeholder
        pred_conf = abs(pred_pct) * 10
        vol_factor = 100 / (indicators['atr']/close * 100 + 1)
        trend_conf = min(indicators['adx'], 50)
        model_agreement = 75.0  # Placeholder: avg of LSTM/Tree models (enhance later)
        confidence = min((pred_conf * 0.3 + vol_factor*0.2 + trend_conf*0.2 + model_agreement*0.3), 100)
        
        return {
            'symbol': symbol,
            'current_price': close,
            'predicted_price': pred_price,
            'prediction_pct': round(pred_pct, 2),
            'signal': final_signal,
